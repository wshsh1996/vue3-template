...
<template>
  <div>
    ...
    <div style="padding-left: 5px; display: flex; justify-content: flex-start; width: 100%">
      <div style="width: 33%">
        用户名称：{{ props.detail?.user.nickname ? props.detail?.user.nickname : '无' }}
      </div>
      <div>绑定电话：{{ props.detail?.user.phone ? props.detail?.user.phone : '无' }}</div>
    </div>
    ...
    >
      ...
      <div style="width: 33%">
        商品总数：{{
          props.detail?.goods.map((item: any) => item.number).reduce((a: any, b: any) => a + b)
        }}
      </div>
      <div style="width: 33%">
        商品价格：{{ props.detail?.amount ? props.detail?.amount + '元' : 0 }}
      </div>
      <div style="width: 33%">
        优惠券金额：{{ props.detail?.coupon_amount ? props.detail?.coupon_amount + '元' : 0 }}
      </div>
      <div style="width: 33%">
        会员折扣：{{ props.detail?.discount_amount ? props.detail?.discount_amount : 0 }}
      </div>
      <div style="width: 33%">
        运费：{{ props.detail?.freight ? props.detail?.freight + '元' : 0 }}
      </div>
      <div style="width: 33%">自提码：{{ props.detail?.code ? props.detail?.code : '-' }}</div>
      <div style="width: 33%">
        实际支付：{{ props.detail?.payment?.amount ? props.detail?.payment.amount + '元' : 0 }}
      </div>
    </div>
    <el-divider border-style="dashed"></el-divider>
    <!--  发货信息  -->
